        ...
        wsClient.onmessage = function(message) {
            ...
            setTimeout(() => {
                ...
                if (msg["event"] === 'game:match_ended') {
                    const winner = {
                        "winner_side" : msg["data"]["winner_team_num"]
                    }
                    console.log("Game Winner is " + msg["data"]["winner_team_num"] + "! Writing to file...");
                    const winner_json = JSON.stringify(winner);
                    fs.writeFile('../matchwinner.json',winner_json,'utf-8', (err) => {
                        if(err) {
                            console.log(err);
                        } else {
                            console.log("Winner has been written to file successfully!");
                        }
                    });
                }
